...
export default async function handler(
  ...
  const prismaNewHandler = async (result: any, pathFile: string) => {
    try {
      ...
      return res.status(500).json({ message: 'Error updating server with new testimony' })
    }
  }
  ...
  const prismaUpdateHandler = async (result: any, pathFile: string) => {
    ...
    try {
      ...
      return res.status(500).json({ message: 'Error updating server with existing testimony' })
    }
  }
